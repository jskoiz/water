...
`RuntimeFeature` accepts either `init` or `mount`, an optional per-frame `update`, an optional `resize`, and `dispose`. Every callback receives the shared scene, camera, renderer, timer, viewport, input snapshot, typed configuration, loading controller, fatal-error seam, and generic service registry. A feature must provide `init` or `mount`; the runtime validates this when starting.

The shared `timer` is a `THREE.Timer` connected to the document visibility lifecycle. The runtime resets it on start/resume, updates it once per animation frame with the `requestAnimationFrame` timestamp, and reads `getDelta()`/`getElapsed()` for frame context. Runtime disposal calls `timer.dispose()` so its visibility listener is released.
